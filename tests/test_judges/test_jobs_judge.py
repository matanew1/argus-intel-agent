...
def _make_mock_judgment(label: str):
    from argus.judges.jobs import JobJudgment
    return JobJudgment(label=label, reasoning="Test reasoning.", confidence=0.85)


@patch("argus.judges.jobs.ChatMistralAI")
def test_judge_job_cluster_building_ai_team(mock_llm_class):
    ...
    result = judge_job_cluster("OpenAI", SAMPLE_ROLES, SAMPLE_CRITERIA)
...
@patch("argus.judges.jobs.ChatMistralAI")
def test_judge_job_cluster_routine(mock_llm_class):
    from argus.judges.jobs import judge_job_cluster
    ...
    result = judge_job_cluster("OpenAI", routine_roles, SAMPLE_CRITERIA)
