...
def _make_mock_judgment(label: str, summary: str = "Price changed."):
    ...
    return DiffSignal(label=label, reasoning="Test reasoning.", summary=summary, confidence=0.9)


@patch("argus.classifiers.diff.ChatMistralAI")
def test_classify_diff_material(mock_llm_class):
    ...
    result = classify_diff("OpenAI", "https://openai.com/pricing", SAMPLE_DIFF)
...
@patch("argus.classifiers.diff.ChatMistralAI")
def test_classify_diff_cosmetic(mock_llm_class):
    from argus.classifiers.diff import classify_diff
    ...
    result = classify_diff("OpenAI", "https://openai.com/pricing", "-teh\n+the")
